...
export const handle = async ( { event, resolve } ) =>
{
  ...
  let response = await resolve( event, {
    transformPageChunk: ( { html } ) => html.replace('%lang%', event.request.headers.get('accept-language') )
  });
...
export const handleFetch = async ( { event, request, fetch } ) =>
{
  ...
  if ( request.url.startsWith( 'http://localhost:5173/searcher' ) )
  {
    event.request.headers.set( 'x-custom-header', 'header for fetch from searcher' );
    request = await new Request( 'http://localhost:5173/searcher', request);
  }
